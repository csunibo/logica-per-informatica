...
theorem union_symmetric: ∀A B, A ∪ B = B ∪ A := by
 assume A : set
 …
 by … it suffices to prove ∀Z, Z ∈ A ∪ B ↔ Z ∈ B ∪ A
 assume Z : set
 we split the proof
 . we need to prove …
   suppose …
   thus by … we proved Z ∈ A ∨ Z ∈ B as H
   ...
   . case a.mp.inr (H: …) -- guardate dove il nome del caso a.mp.inr compare nella finestra di destra
     we need to prove Z ∈ B ∪ A
...
theorem union_emptyset: ∀A, A ∪ ∅ = A := by
 assume …
 by … it suffices to prove ∀Z, … ↔ …
 assume Z : set
 we split the proof
 . suppose Z ∈ A ∪ ∅
   thus by ax_union1 we proved … as H
   we proceed by cases on H to prove Z ∈ A
   . case a.mp.inl (K: Z ∈ A)
     thus done
   . case a.mp.inr (K: Z ∈ ∅)
     thus by … done
...
theorem exists_subset₁: ∀A, ∃B, B ⊆ A := by
 assume A: set
 we choose ∅ and prove ∅ ⊆ A that is equivalent to …
 …
...
theorem exists_subset₂: ∀A, ∃B, B ⊆ A := by
 assume A: set
 we choose A and prove … that is equivalent to …
 …
...
theorem intersect_union₁: ∀A B C, A ∩ (B ∪ C) ⊆ A ∩ B ∪ A ∩ C := by
 ...
 assume B: set
 assume C: set
 we need to prove A ∩ (B ∪ C) ⊆ A ∩ B ∪ A ∩ C that is equivalent to …
 assume …
 suppose …
 thus by … we proved … as H₁ and … as H₂
 thus by … we proved … ∨ … as H₂'
 we proceed by cases on H₂' to prove Z ∈ A ∩ B ∪ A ∩ C
 . case intro.inl (K: Z ∈ B)
   thus by … we proved Z ∈ A ∩ B
   thus by … done
 . …
